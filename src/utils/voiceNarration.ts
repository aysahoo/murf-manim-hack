...
export async function generateNarrationScript(topic: string, manimCode: string): Promise<{
  ...
  try {
    ...
    if (cachedScript) {
      console.log(`🎯 Cache HIT - Using cached voice script for: ${topic}`);
      return cachedScript;
    }

    console.log(`🔄 Cache MISS - Generating new voice script for: ${topic}`);
    ...
    console.log(`💾 Cached fallback voice script for future use: ${topic}`);
...
Keep it simple, clear, and engaging for general audience.`
    ...
    console.log(`💾 Cached voice script for future use: ${topic}`);
