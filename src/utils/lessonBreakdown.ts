...
export async function generateLessonBreakdown(topic: string): Promise<{
  ...
  try {
    ...
    // Generate AI-powered lesson breakdown
...
Topic: ${topic}`
    ...
    throw new Error(`Failed to generate lesson breakdown for topic: ${topic}. Original error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
