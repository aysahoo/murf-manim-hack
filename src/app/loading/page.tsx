...
const CounterAnimation = ({ onComplete, topic, mode }: { onComplete: () => void; topic: string; mode: string }) => {
  const [count, setCount] = useState(0);
  const [message, setMessage] = useState("It's being cooked...");

  const funMessages = mode === "lessons" ? [
      ...
      try {
        if (mode === "lessons") {
          ...
          if (data && data.lessons && Array.isArray(data.lessons)) {
            sessionStorage.setItem('generatedLessons', JSON.stringify({
              topic: data.topic,
          ...
          if (data.success && data.videoUrls?.[0]) {
            sessionStorage.setItem('generatedVideo', JSON.stringify({
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex flex-col w-full">
          <div className="flex-1 flex flex-col justify-center items-center w-full">
                ...
                {/* Counter animation */}
                <CounterAnimation onComplete={handleComplete} topic={topic || ""} mode={mode} />
              </div>
          </div>
        </main>
      </div>
